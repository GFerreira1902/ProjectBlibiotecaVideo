...
const FilmeEdit = () => {
  ...
  function salvarfilme(dados) {
    if (params.id) {
        FilmeService.update(params.id, dados)
        Swal.fire({
            icon: 'success',
  ...
  useEffect(() => {
    ...
    setCategorias(FilmeService.getAll())
      ...
      if (params.id) {
          const filme = FilmeService.get(params.id)
  ...
  return (
      <div align='center' style={{color:'black'}}>
        <Col md={8} className='mt-3'>
          ...
          <Button variant="primary" onClick={handleSubmit(salvarfilme)}>
              Alterar
          </Button>
        </Col>
      </div>     
