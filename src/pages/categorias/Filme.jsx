...
const Filme = () => {
  ...
  function apagar(id) {
    if(window.confirm('Deseja realmente excluir o registro?')) {
      ...
      setCategorias(FilmeService.getAll())
    }
  }
  ...
  return (
    <div>
      <Col lg={1}>
        <Button variant='dark' onClick={handleShowCat} className='float-end'>Adicionar Filme<IoMdAddCircleOutline className="fs3" /></Button>
      </Col>
      <Row>
        
          <h2 className='mt-5 mb-3'>Filmes</h2>
          {categorias.map((categoria,i) => (
            <Card style={{ backgroundColor:'#000000', width: '250px', height: '280px', margin: '10px', marginTop: '8%',color: 'white', textAlign: 'left', boxShadow: '20px 10px 10px black', borderRadius:'35px',justifyContent:'center' }} border='primary' key={i}>
              ...
              <Card.Body>
                <Link to={"/categoria/filme/"+i} style={{textDecoration:'none',color:'white',}}><Card.Title style={{fontSize:'15px', paddingBottom:'40px'}} >{categoria.nome}</Card.Title></Link>
                <BsTrash onClick={() => apagar(i)} className='text-danger fs-5 mt-2 float-end' />
                <a href={'/categoria/filme/edit/'+i}><BsPencilFill className='text-light fs-5' style={{justifyContent:'right'}}/></a>                
              </Card.Body>
            </Card>
          ))}
      </Row>
        ...
        <Modal.Body style={{ backgroundColor: "black", color: "white" }}>
          <Form>
            <Form.Group className="mb-3" controlId="video">
              ...
              <Form.Control isInvalid={errors.video} type="text" {...register("video", FilmeValidator.video)} />
            ...
            <Form.Group className="mb-3" controlId="thumb">
              ...
              <Form.Control isInvalid={errors.thumb} type="text" {...register("thumb", FilmeValidator.thumb)} />
              {errors.thumb && <span className="text-danger">{errors.thumb.message}</span>}
            </Form.Group>         
            <Form.Group className="mb-3" controlId="nome">
              <Form.Label>Nome ou Título: </Form.Label>
              <Form.Control isInvalid={errors.nome} type="text" {...register("nome", FilmeValidator.nome)} />
