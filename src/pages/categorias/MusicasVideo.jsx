...
const MusicasVideo = () => {
    ...
    const handleMusicas = () => setShowLan(false);
    ...
    useEffect(() => {
        setVideos (MusicasService.get(params.id))
        setMusicas(MusicasService.getAll())
        if (params.id) {
            const video = MusicasService.get(params.id)
    ...
    return (
        <div className="mt-5" style={{color:'black'}} >
            ...
            <Row>
                ...
                <Col md={5} >
                    {musicas.map((musicas, i) => (
                       
                        <a href={"/musicas/"+i} >
                            <Card style={{ backgroundColor: '#000000', width: '350px', height: '245px', margin: '10px', color: 'white', textAlign: 'center', borderRadius: '35px', justifyContent: 'center' }} border='primary' key={musicas.id} href="#">
                                <Card.Img variant='top' src={musicas.thumb} style={{ width: '100%', height: '70%', borderRadius: '35px' }} />
                                <Card.Body>
                                    <Card.Title style={{ fontSize: '15px', }} >{musicas.nome}</Card.Title>                                                               
                                </Card.Body>
                            </Card>
                        </a>
            ...
            <Modal show={showlan} onHide={handleMusicas} >
